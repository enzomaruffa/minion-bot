...
def show_list(list_type: str | None = None, include_checked: bool = False) -> str:
    ...
    with session_scope() as session:
        ...
        if not items:
            if lt:
                return f"No items in {lt.value.title()} list. Try 'add to {lt.value}' to add one!"
            return "All shopping lists are empty. Try 'add milk to groceries' to get started!"
        ...
        for item_type in [ShoppingListType.GIFTS, ShoppingListType.GROCERIES, ShoppingListType.WISHLIST]:
            ...
            for item in grouped[item_type]:
                checkbox = "[x]" if item.is_complete else "[ ]"
                ...
                lines.append(f"  {checkbox} #{item.id}: {item.name}{qty_info}{recipient_info}{notes_info}")
...
def check_item(item_id: int) -> str:
    ...
    with session_scope() as session:
        ...
        if success:
            return f"Checked off: #{item_id} {name}"
...
def purchase_item(item_id: int, quantity: int = 1) -> str:
    ...
    with session_scope() as session:
        ...
        if success:
            if purchased >= target:
                return f"{name} complete! ({purchased}/{target})"
...
def clear_checked(list_type: str | None = None) -> str:
    ...
    with session_scope() as session:
        ...
        list_info = f" from {lt.value.title()}" if lt else ""
...
def show_gifts_for_contact(contact_name: str, include_checked: bool = False) -> str:
    ...
    with session_scope() as session:
        ...
        for item in gifts:
            ...
            notes_info = f" - {item.notes}" if item.notes else ""
            qty_info = f" ({item.quantity_purchased}/{item.quantity_target})" if item.quantity_target > 1 else ""
            lines.append(f"  {checkbox} #{item.id}: {item.name}{qty_info}{notes_info}")
